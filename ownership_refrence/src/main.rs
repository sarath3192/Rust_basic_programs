...
// fn main(){
//    let pid_m = unsafe{ getpid()};
//    let tid_m = unsafe{ syscall(SYS_gettid)   };
//    println!("main_pid: {}", pid_m);
//    println!("main_tid: {}", tid_m);
...
async fn print_ids(task: &str) {
    let pid = unsafe { getpid() };
    let tid = unsafe { syscall(SYS_gettid) };
    println!("{} → PID: {}, TID: {}", task, pid, tid);
    sleep(Duration::from_millis(100)).await;
}

#[tokio::main]
async fn main() {
    tokio::spawn(print_ids("Task 1")).await.unwrap();
    tokio::spawn(print_ids("Task 2")).await.unwrap();
}
